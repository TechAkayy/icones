  ...
  <WithNavbar>
    ...
    <div mb--3 md:mx-6 md:mt-6>
      <div class="border-b py-3  flex md:shadow md:rounded outline-none md:py-1 px-4 border-x border-b md:border-t border-base">
        ...
        <form action="/collection/all" class="flex-auto" role="search" method="get" @submit.prevent>
          <input ref="input" v-model="categorySearch" aria-label="Search" class="text-base outline-none w-full py-1 px-4 m-0 bg-transparent" name="s" placeholder="Search category..." autofocus autocomplete="off" @keydown="onKeydown">
        </form>
        ...
        <button
          class="flex items-center transition ml-4" :class="{
      ...
      <RouterLink :class="categorySearch ? '' : 'op0 pointer-events-none'" px4 py2 w-full mt--1px text-sm z--1 h-10 flex="~ gap-2" items-center border="~ base rounded-b" hover="text-primary !border-primary shadow" :to="`/collection/all?s=${categorySearch}`">
    ...
    <template v-for="c of categorized" :key="c.name">
      <div v-if="(c.collections).length" px4>
        ...
        <CollectionEntries of-hidden :collections="c.collections" :type="c.type" />
      </div>
    </template>
    <div v-if="categorized.every(c => !c.collections.length)" class="flex flex-col flex-grow w-full py-6 justify-center items-center">
      <Icon icon="ph:x-circle-bold" class="text-4xl mb-2 opacity-20" /><span class="text-lg opacity-60">There is no result corresponding to your search query.</span>
    </div>
    <Footer />
  </WithNavbar>
</template>
